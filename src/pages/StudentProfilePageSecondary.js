...
const StudentProfilePageSecondary = ({studentId}) => {
    ...
    const getProfile = async () => {
      try {
        if (auth.role === "Student") {
          await apiPrivate.get("student/getStudent/" + studentId).then((res) => {
            if (res.status === 200) {
              setProfile(res.data)
            }
          })
          ...
          await apiPrivate.get("teacher/getStudent/" + studentId).then((res) => {
            if (res.status === 200) {
              setProfile(res.data)
            }
          })
